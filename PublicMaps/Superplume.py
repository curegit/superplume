# Superplume.py: Perfect World 2 fork
...
# program; if not, write to the Free Software Foundation, 51 Franklin Street, Fifth
...
class MapConstants:
    ...
    def initialize(self):
        ...
        self.HillChanceAtOne = 0.50
        self.PeakChanceAtOne = 0.27
...
class HeightMap:
    ...
    def ensureWaterAndIceAtEdges(self):
        gc = CyGlobalContext()
        mmap = gc.getMap()
        # 上端と下端の少なくとも2タイル分を水域にし、氷河を配置させる
        for y in range(2):
            for x in range(mc.hmWidth):
                i = GetHmIndex(x, y)
                if y == 0:
        ...
        for i in range(mc.hmWidth * mc.hmHeight):
            ...
            for plateID in range(1, mc.hmNumberOfPlates + 1):
                ...
                ripple = (
                    ...
                    * mc.rippleAmplitude
                    * math.sin(math.radians(angleDifference))
                )
    ...
    def createContinentList(self, ID):
        ...
        for y in range(0, mc.hmHeight):
            for x in range(0, mc.hmWidth):
                ...
                if y % gap == 0 and x % gap == 0 and self.areaMap.areaMap[i] == ID:
        ...
        for xRightExtreme in range(seg.xLeft + lineFound - landOffset, xStop, 1):
            ...
            if self.areaMap[i] == 0 and matchValue == matchFunction(
                ...
                if xRightExtreme >= seg.xRight + landOffset:
                    ...
                    break
                    # past the end of the parent line and this line ends
            elif lineFound == False and xRightExtreme >= seg.xRight + landOffset:
                if debugReport:
                    print "no additional lines found"
                break
                # past the end of the parent line and no line found
            else:
...
class StartingPlotFinder:
    ...
    def getPlotPotentialValue(self, x, y, coastalCity):
        ...
        for n in range(gc.getNumBuildInfos()):
            ...
            if (
                ...
                if debugOut:
                    print "Tech is high enough for %(s)s" % {"s": impInfo.getType()}
        ...
        if len(improvementList) > 0:
            ...
            if debugOut:
                print "bestImp.value=%(b)d" % {"b": bestImp.value}
    ...
    def boostCityPlotValue(self, x, y, bonuses, isCoastalCity):
        ...
        debugOut = False
        if debugOut:
            print "Value before boost = %(v)d" % {"v": value}
        gc = CyGlobalContext()
        gameMap = CyMap()
        game = gc.getGame()
        ...
        for n in range(len(yields)):
            for i in range(gc.getNUM_CITY_PLOTS()):
                ...
                if debugOut:
                    print "value now at %(v)d" % {"v": value}
                if bonusCount >= bonuses:
                    if debugOut:
                        print "Placed all bonuses."
                    if debugOut:
                        print "****************************************************"
                    return
                plot = plotCity(x, y, i)
        ...
        if debugOut:
            print "Failed to boost city value. value= %(v)d" % {"v": value}
        if debugOut:
            print "****************************************************"
        return
    ...
    def ensureMinimumHills(self, x, y):
        ...
        if hillsNeeded > 0:
            for plot in plotList:
                ...
                if (
                    plot.getPlotType() != PlotTypes.PLOT_HILLS
                    and plot.getArea() == gameMap.plot(x, y).getArea()
                    and (featureInfo == None or not featureInfo.isRequiresFlatlands())
                    and (bonusInfo == None or not bonusInfo.isRequiresFlatlands())
                ):
                    plot.setPlotType(PlotTypes.PLOT_HILLS, True, True)
                    hillsNeeded -= 1
                    print "adding hill"
